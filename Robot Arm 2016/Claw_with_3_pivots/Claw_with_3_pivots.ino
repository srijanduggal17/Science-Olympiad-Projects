...
void setup() {
  ...
  previous2 = map(analogRead(A3),0,1023,0,200);
...
void loop(){
  ...
  int val2 = map(analogRead(A3),0,1023,0,200);
  // move a number of steps equal to the change in the
  // sensor reading
  int steps = val2 - previous2;
  //Serial.println(steps);
  if (steps > 0)
  {
     myMotor1->step(steps, FORWARD, SINGLE);
  }
  else if (steps < 0)
  {
     myMotor1->step(-steps, BACKWARD, SINGLE);
  }
  // remember the previous value of the sensor
  previous2 = val2;
